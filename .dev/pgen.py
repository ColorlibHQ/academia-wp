#!/usr/bin/env python3
"""Emitters for Academia block-pattern PHP files.
...
def separator(*, style="wide", color="divider"):
...
def grid(inner, *, cols, gap=ROW_GAP):
    ...
    return group(inner, align="wide", layout="grid", col_count=cols, gap=gap,
                 class_name=f"academia-grid-{cols}" if cols in (2, 3, 4) else None)
...
def faq_list(pairs):
    ...
    items = [details(q, para(a, color="muted"), class_name="is-style-academia-faq")
             for q, a in pairs]
...
def band(title, body, buttons_list, *, width="720px"):
    ...
    inner = (heading(title, align="center", size="huge", color="base") + "\n" +
    ...
    return section_std(
        ...
        gradient="primary-to-deep", text="base", gap="0", elements=GRADIENT_ELEMENTS)
