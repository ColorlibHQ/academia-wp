 ...
 * call our filter.
 */
const ACADEMIA_UPDATE_HOST     = 'updates.colorlib.com';
const ACADEMIA_UPDATE_ENDPOINT = 'https://updates.colorlib.com/theme/academia.json';
const ACADEMIA_UPDATE_CACHE    = 'academia_update_response';
 ...
 * @return bool
 */
function academia_updates_enabled() {
	/**
	 * Filters whether Academia checks for updates.
	 *
	 ...
	 */
	return (bool) apply_filters( 'academia_check_for_updates', true );
}
 ...
 * @return array
 */
function academia_update_payload() {
	$payload = array(
		'theme'      => 'academia',
	 ...
	 */
	return apply_filters( 'academia_update_payload', $payload );
}
 ...
 * @return array|null
 */
function academia_fetch_update() {
	if ( ! academia_updates_enabled() ) {
		return null;
	}

	$cached = get_site_transient( ACADEMIA_UPDATE_CACHE );
	...
	$response = wp_remote_get(
		add_query_arg( academia_update_payload(), ACADEMIA_UPDATE_ENDPOINT ),
		array(
			'timeout'    => 8,
			'user-agent' => academia_update_user_agent(),
		)
	);

	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		set_site_transient( ACADEMIA_UPDATE_CACHE, array(), HOUR_IN_SECONDS );
		return null;
	}
	...
	set_site_transient( ACADEMIA_UPDATE_CACHE, $data, 12 * HOUR_IN_SECONDS );
 ...
 * @return array|false
 */
function academia_check_update( $update, $theme_data, $theme_stylesheet ) {
	if ( $update || 'academia' !== $theme_stylesheet ) {
		return $update;
	}

	$release = academia_fetch_update();

	if ( ! $release || version_compare( $release['version'], ACADEMIA_VERSION, '<=' ) ) {
		return $update;
	}

	return array(
		'id'           => ACADEMIA_UPDATE_ENDPOINT,
		'theme'        => $theme_stylesheet,
		'version'      => $release['version'],
		'url'          => isset( $release['url'] ) ? $release['url'] : 'https://colorlib.com/wp/themes/academia/',
...
add_filter( 'update_themes_' . ACADEMIA_UPDATE_HOST, 'academia_check_update', 10, 3 );

/**
 * Forget the cached response when someone asks WordPress to check again.
 */
function academia_flush_update_cache() {
	delete_site_transient( ACADEMIA_UPDATE_CACHE );
}
add_action( 'upgrader_process_complete', 'academia_flush_update_cache' );
add_action( 'after_switch_theme', 'academia_flush_update_cache' );
 ...
 * their server.
 */
function academia_updates_notice() {
	$screen = get_current_screen();

	if ( ! $screen || 'appearance_page_academia-starter-sites' !== $screen->id ) {
		return;
	}
	...
	$payload = academia_update_payload();
	...
	$note = function_exists( 'academia_library_check_update' )
		/* translators: 1: the update host, 2: the list of values sent, 3: filter name. */
		? __( 'Academia and its Starter Library plugin check %1$s for updates twice a day and send %2$s. No personal data and no site name is sent, and the identifier is a one-way hash. Add the %3$s filter to switch both off.', 'academia' )
		/* translators: 1: the update host, 2: the list of values sent, 3: filter name. */
		: __( 'Academia checks %1$s for updates twice a day and sends %2$s. No personal data and no site name is sent, and the identifier is a one-way hash. Add the %3$s filter to switch it off.', 'academia' );
	?>
	<p class="description academia-updates-note">
		<?php
		printf(
			esc_html( $note ),
			'<code>' . esc_html( ACADEMIA_UPDATE_HOST ) . '</code>',
			'<code>' . esc_html( implode( ', ', array_keys( $payload ) ) ) . '</code>',
			'<code>academia_check_for_updates</code>'
		);
		?>
	</p>
	<style>.academia-updates-note { max-width: 70ch; margin-top: 18px; }</style>
	<?php
}
add_action( 'admin_footer', 'academia_updates_notice' );
 ...
 * @return string
 */
function academia_update_user_agent() {
	return 'Academia/' . ACADEMIA_VERSION . '; WordPress/' . get_bloginfo( 'version' );
}
 ...
 * @return array
 */
function academia_update_request_args( $args, $url ) {
	if ( 'updates.colorlib.com' === wp_parse_url( $url, PHP_URL_HOST ) ) {
		$args['user-agent'] = academia_update_user_agent();
	}
	return $args;
}
add_filter( 'http_request_args', 'academia_update_request_args', 10, 2 );
