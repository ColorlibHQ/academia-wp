 ...
 * WordPress.org rejects themes that do it. So Academia does the next best thing:
...
/**
 * The form plugins Academia knows how to render, in the order they are preferred.
 *
 ...
 * @return array[]
 */
function academia_form_providers() {
	$providers = array(
		'wpforms'     => array(
			'label'     => 'WPForms',
		...
		'gravity'     => array(
			...
			'callback'  => 'academia_gravity_first_form',
		...
		'fluent'      => array(
			...
			'callback'  => 'academia_fluent_first_form',
		...
		'ninja'       => array(
			...
			'callback'  => 'academia_ninja_first_form',
	...
	/**
	 * Filters the form plugins Academia can render.
	 *
	 * @param array[] $providers Provider definitions.
	 */
	return apply_filters( 'academia_form_providers', $providers );
}
 ...
 * @return bool
 */
function academia_form_provider_active( $provider ) {
 ...
 */
function academia_form_provider_id( $provider ) {
 ...
 * @return int
 */
function academia_gravity_first_form() {
 ...
 * @return int
 */
function academia_fluent_first_form() {
	global $wpdb;
 ...
 * @return int
 */
function academia_ninja_first_form() {
...
function academia_detect_form() {
	...
	foreach ( academia_form_providers() as $key => $provider ) {
		if ( ! academia_form_provider_active( $provider ) ) {
			continue;
		}
		...
		$id = academia_form_provider_id( $provider );
		if ( ! $id ) {
			continue;
		}
	...
	/**
	 * Filters the form Academia renders in its contact patterns.
	 *
	 ...
	 */
	return apply_filters( 'academia_detected_form', $detected );
}
 ...
 */
function academia_contact_form( $args = array() ) {
	$args = wp_parse_args(
		$args,
		array(
			'title'    => _x( 'Send a message', 'Contact form heading', 'academia' ),
			'email'    => 'hello@example.com',
			'fallback' => _x( 'The quickest way to reach us is email — we answer within two working days.', 'Contact fallback text', 'academia' ),
		)
	);

	$form = academia_detect_form();
	...
	if ( $form && ! empty( $form['markup'] ) ) {
		...
		return $out
			. '<!-- wp:group {"className":"academia-form","layout":{"type":"default"}} -->'
			. '<div class="wp-block-group academia-form">' . $inner . '</div>'
			. '<!-- /wp:group -->';
	}
	...
	if ( current_user_can( 'edit_theme_options' ) ) {
		$out .= '<!-- wp:paragraph {"textColor":"muted","fontSize":"small"} -->'
			...
			. esc_html__( 'Only you can see this: install any form plugin — WPForms, Contact Form 7, Gravity Forms, Fluent Forms and several others are recognised — and its form replaces this panel automatically.', 'academia' )
			. '</em></p><!-- /wp:paragraph -->';
	}
 ...
 */
function academia_form_styles() {
	if ( ! academia_detect_form() ) {
		return;
	}
...
add_action( 'wp_enqueue_scripts', 'academia_form_styles' );
