 ...
 *   add_filter( 'academia_auto_setup_front_page', '__return_false' );
 *
 * @package Academia
 * @since   2.0.0
 */
...
const ACADEMIA_SETUP_OPTION = 'academia_front_page_setup';

/** Option flag: offer the one-click setup notice (site already had a front page). */
const ACADEMIA_OFFER_OPTION = 'academia_offer_front_page_setup';

/** Transient: result notice to show once after a setup run ("done" | "error"). */
const ACADEMIA_RESULT_TRANSIENT = 'academia_front_page_setup_result';
 ...
 *
 * @param string $slug  Pattern slug, e.g. `academia/page-home`.
 * @param int    $depth Recursion guard.
 * @return string Block markup, or empty string when the pattern is unknown.
 */
function academia_get_pattern_markup( $slug, $depth = 0 ) {
	if ( $depth > 5 ) {
		return '';
	}
	...
	return preg_replace_callback(
		...
		static function ( $matches ) use ( $depth ) {
			...
			return academia_get_pattern_markup( $attrs['slug'], $depth + 1 );
		},
		$content
	);
}
 ...
 */
function academia_setup_front_page() {
	$state = get_option( ACADEMIA_SETUP_OPTION, array() );
	...
	if ( ! $home_id || 'page' !== get_post_type( $home_id ) || 'publish' !== get_post_status( $home_id ) ) {
		$content = academia_get_pattern_markup( 'academia/page-home' );
		if ( '' === $content ) {
			if ( $kses_active ) {
				kses_init_filters();
			}
			return new WP_Error( 'academia_missing_pattern', __( 'The "Home landing page" pattern could not be loaded.', 'academia' ) );
		}
		...
		$home_id = wp_insert_post(
			array(
				'post_type'      => 'page',
				'post_status'    => 'publish',
				'post_title'     => _x( 'Home', 'Title of the front page created on activation', 'academia' ),
				'post_name'      => 'home',
 ...
 */
function academia_after_switch_theme() {
	if ( get_option( ACADEMIA_SETUP_OPTION ) ) {
	...
	if ( ! $has_static_front && apply_filters( 'academia_auto_setup_front_page', true ) ) {
		update_option( 'academia_pending_front_page_setup', 1 );
		return;
	}

	update_option( ACADEMIA_OFFER_OPTION, 1 );
}
add_action( 'after_switch_theme', 'academia_after_switch_theme' );

/**
 * Run a pending automatic setup once patterns are available.
 */
function academia_run_pending_setup() {
	if ( ! get_option( 'academia_pending_front_page_setup' ) ) {
		return;
	}
	delete_option( 'academia_pending_front_page_setup' );

	if ( get_option( ACADEMIA_SETUP_OPTION ) ) {
		return;
	}

	$result = academia_setup_front_page();
	set_transient( ACADEMIA_RESULT_TRANSIENT, is_wp_error( $result ) ? 'error' : 'done', HOUR_IN_SECONDS );
}
add_action( 'init', 'academia_run_pending_setup', 100 );

/**
 * Forget the pending/offer state when the theme is switched away.
 */
function academia_switch_theme_cleanup() {
	delete_option( 'academia_pending_front_page_setup' );
	delete_option( ACADEMIA_OFFER_OPTION );
	delete_transient( ACADEMIA_RESULT_TRANSIENT );
}
add_action( 'switch_theme', 'academia_switch_theme_cleanup' );

/**
 * Handle the "Set up front page" button (admin-post).
 */
function academia_handle_setup_request() {
	check_admin_referer( 'academia_setup_front_page' );
	if ( ! current_user_can( 'edit_theme_options' ) ) {
		wp_die( esc_html__( 'You are not allowed to change the front page.', 'academia' ), 403 );
	}
...
add_action( 'admin_post_academia_setup_front_page', 'academia_handle_setup_request' );

/**
 * Handle the "Not now" link (admin-post).
 */
function academia_handle_dismiss_request() {
	check_admin_referer( 'academia_dismiss_front_page_setup' );
	if ( ! current_user_can( 'edit_theme_options' ) ) {
		wp_die( esc_html__( 'You are not allowed to change the front page.', 'academia' ), 403 );
	}
...
add_action( 'admin_post_academia_dismiss_front_page_setup', 'academia_handle_dismiss_request' );

/**
 * Admin notices: setup result, or the offer to set up.
 */
function academia_front_page_notices() {
	if ( ! current_user_can( 'edit_theme_options' ) ) {
		return;
	}
	...
	$result = get_transient( ACADEMIA_RESULT_TRANSIENT );
	if ( $result ) {
		delete_transient( ACADEMIA_RESULT_TRANSIENT );
		$state = get_option( ACADEMIA_SETUP_OPTION, array() );
		...
		if ( 'done' === $result && ! empty( $state['home'] ) ) {
			printf(
				...
				esc_html__( 'Academia is ready.', 'academia' ),
				esc_html__( 'A "Home" page with the Academia landing page and a "Blog" page were created and assigned in Settings → Reading. Everything on the front page is editable as normal page content.', 'academia' ),
				esc_url( home_url( '/' ) ),
				esc_html__( 'View front page', 'academia' ),
				esc_url( get_edit_post_link( absint( $state['home'] ) ) ),
				esc_html__( 'Edit front page', 'academia' ),
				esc_url( admin_url( 'site-editor.php' ) ),
				esc_html__( 'Open Site Editor', 'academia' )
			);
		} else {
			printf(
				'<div class="notice notice-error is-dismissible"><p>%s</p></div>',
				esc_html__( 'Academia could not create the front page automatically. Create a page, insert the "Home landing page" pattern and choose it under Settings → Reading.', 'academia' )
			);
		}
		return;
	}

	if ( ! get_option( ACADEMIA_OFFER_OPTION ) ) {
		return;
	}

	$front_id    = absint( get_option( 'page_on_front' ) );
	$front_title = $front_id ? get_the_title( $front_id ) : '';
	$setup_url   = wp_nonce_url( admin_url( 'admin-post.php?action=academia_setup_front_page' ), 'academia_setup_front_page' );
	$dismiss_url = wp_nonce_url( admin_url( 'admin-post.php?action=academia_dismiss_front_page_setup' ), 'academia_dismiss_front_page_setup' );
	...
	printf(
		...
		esc_html__( 'Thanks for choosing Academia!', 'academia' ),
		$front_title
			/* translators: %s: title of the current front page. */
			? sprintf( esc_html__( 'Your site already uses "%s" as its front page, so it was left untouched. Academia can create a "Home" page with its landing-page design and make it the front page — you can switch back any time under Settings → Reading.', 'academia' ), esc_html( $front_title ) )
			: esc_html__( 'Academia can create a "Home" page with its landing-page design and a "Blog" page, and set them under Settings → Reading.', 'academia' ),
		esc_url( $setup_url ),
		esc_html__( 'Set up the Academia front page', 'academia' ),
		esc_url( $dismiss_url ),
		esc_html__( 'Not now', 'academia' ),
		esc_url( admin_url( 'themes.php?page=academia-starter-sites' ) ),
		esc_html__( 'Choose a starter site', 'academia' )
	);
}
add_action( 'admin_notices', 'academia_front_page_notices' );
